      ...
      void Velodyne16::msg2Cloud(const std::vector<uint8_t> &pkt_msg_buf, pcl::PointCloud<pcl::PointXYZITCA> &out_cloud)
      {
        ...
        for (uint32_t blk_idx = 0; blk_idx < NUM_BLOCK; blk_idx++) //data block 0~11
        {
          ...
          for (uint32_t chan_idx = 0; chan_idx < NUM_LIDAR_CHANNELS; chan_idx++) //channel 0 ~ 15 azimoth n
          {
            ...
            std::cout<< "range1_idx: " <<range1_idx << std::endl;
            std::cout<< "range2_idx: " <<range2_idx << std::endl;
            ...
            // hex.append(pkt_ptr->blocks[blk_idx].firing1[range2_idx]);
            // hex.append(pkt_ptr->blocks[blk_idx].firing1[range1_idx]);
            ...
            // getRange(const char *tmp1, const char *tmp2);
          ...
          for (uint32_t chan_idx = 0; chan_idx < NUM_LIDAR_CHANNELS; chan_idx++) //channel 0 ~ 15 azimoth n
          {
            ...
            char *tmp1 = (char *)&pkt_ptr->blocks[blk_idx].firing2[range2_idx];
            char *tmp2 = (char *)&pkt_ptr->blocks[blk_idx].firing2[range1_idx];
            ...
            double intensity = pkt_ptr->blocks[blk_idx].firing2[reflectivity_idx];
