  ...
  namespace backend
  {
    ...
    void InputCloud::addVelocity(const sensor::Velocity &vel)
    {
      ...
      while (!velocity_queue_.empty())
      {
        ...
        break; // ? break 안하면 while문을 못빠져나옴. 왜?
      }
    }
...
// std::cout << "last_point_time_stamp_sec: " << last_point_time_stamp_sec << std::endl;
      ...
      // std::cout << "velocity_it->header.stamp = " << rclcpp::Time(velocity_it->header.stamp).seconds() << std::endl;
      ...
      for (; point_it != cloud.begin(); --point_it)
      {
        for (;
             ...
             ++velocity_it)
        {
          // std::cout << "velocity_it->linear_x = " << velocity_it->linear_x << std::endl;
          // std::cout << "velocity_it->angular_z = " << velocity_it->angular_z << std::endl;
        }
        ...
        if (time_diff > 0.1)
        {
          // std::cout << "point_it->timestamp: " << point_it->timestamp <<std::endl;
          // std::cout << "velocity_it->header.stamp: " << rclcpp::Time(velocity_it->header.stamp).seconds() << std::endl;
...
//     bool InputCloud::deblurringPointCloudMinus(pcl::PointCloud<PointT> &cloud)
//     {
//       if (cloud.empty() || velocity_queue_.empty())
//       {
//         // std::cerr << "cloud or velocity_queue_ is empty." << std::endl;
...
//       {
//         // std::cout << "\n\nstart\n";
//         // std::cout <<"point timestamp: "<< point_it->timestamp <<std::endl;
//         for (velocity_it = velocity_queue_.begin(); 
//             (velocity_it != velocity_queue_.end() - 1 && 
//             point_it->timestamp > rclcpp::Time(velocity_it->header.stamp).seconds());
//             ++velocity_it)
//         {
//           // std::cout << "velocity timestamp: " << rclcpp::Time(velocity_it->header.stamp).seconds() <<std::endl;
//           // std::cout << "velocity_it->linear_x = " << velocity_it->linear_x << std::endl;
//           // std::cout << "velocity_it->angular_z = " << velocity_it->angular_z << std::endl;
//           ;
//         }
...
//         const double time_offset = static_cast<double>(prev_time_stamp_sec - point_it->timestamp);
//         if (time_offset < 0)
//         {
//           std::cout << "Time offset < 0" << std::endl;
//           std::cout << "Time offset < 0" << std::endl;
